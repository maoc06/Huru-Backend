...
function makeRegister({ authDb, handleToken, sendWelcomeMail }) {
  return async function register(userInfo) {
    await validate(userInfo);
    const user = (0, _entities.makeUser)(userInfo);
    const userTmp = { ...user }; // Hash password

    if (userInfo.password) {
      const salt = await _bcryptjs.default.genSalt(10);
      const hashPassword = await _bcryptjs.default.hash(
        userInfo.password,
        salt
      );
    ...
    const verifyEmailToken = handleToken(
      ...
      _config.config.emailPrivateKey,
      '1d'
    );
    sendWelcomeMail({
      emailToSend: userTmp.email,
      firstname: userTmp.firstName,
      url: `https://${_config.config.webAppServerBaseUrl}/verify-email/${verifyEmailToken}`,
    });
