...
var _findImages = _interopRequireDefault(require("../utils/findImages"));

var _findFeatures = _interopRequireDefault(require("../utils/findFeatures"));
...
function makeCarDb({
  ...
  function findById(carId) {
    return Car.findOne({
      ...
      attributes: {
        exclude: ['makerId', 'modelId', 'owner', 'advanceNoticeId', 'minTripDurationId', 'maxTripDurationId', 'fuelId', 'cityId']
      },
      include: [{
        model: AdvanceNotice,
        as: 'advanceNotice'
        ...
        model: Model,
        attributes: ['modelId', 'name', // 'categoryId',
        'numOfSeats', 'transmissionId'],
        include: [{
          model: ModelCategory,
          as: 'categories',
          attributes: {
        ...
        model: User,
        as: 'userOwner',
        attributes: ['uuid', 'firstName', 'lastName', 'profilePhoto', 'createdAt']
      }, {
        model: CarReview,
        as: 'reviews',
        attributes: {
          exclude: ['carId', 'addedBy']
        },
        include: {
          model: User,
          as: 'reviewBy',
  ...
  function findByOwner(owner) {
    return Car.findAll({
      where: {
        owner,
        status: {
          [_sequelize.Op.or]: [CAR_ENABLED_ID, CAR_HIDDEN_ID]
        }
      },
  ...
  async function findCarImages(carId, isMultiple) {
    const carImages = (0, _car.carImageModel)({
      client
    });
    let res = {};

    if (isMultiple) {
      res = await carImages.findAll({
        where: {
          carId
        },
        raw: true
  ...
  async function findByAvailability(city, checkIn, checkOut) {
    const cars = await client.query(`SELECT 
        car.car_id,  
        maker.name, 
        model.model_id,
        model.model,
    ...
    let res = await (0, _findImages.default)({
      arr: cars,
      funcSeeker: findCarImages
    });
    res = await (0, _findFeatures.default)({
      arr: res,
