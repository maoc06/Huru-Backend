...
exports.default = makePaymentGateway;
...
function makePaymentGateway({
  ...
  const paymentGateway = _config.config.payBaseUrl;

  async function generateAcceptanceToken() {
    const response = await (0, _nodeFetch.default)(`${paymentGateway}/merchants/${_config.config.payPublicKey}`, {
      method: 'GET'
    });
  ...
  async function tokenizationSourceCard(cardInfo) {
    const response = await (0, _nodeFetch.default)(`${paymentGateway}/tokens/cards`, {
      method: 'POST',
      body: JSON.stringify(cardInfo),
      headers: {
        Authorization: `Bearer ${_config.config.payPublicKey}`
      }
    });
    const data = await response.json();
    return data;
  ...
  async function tokenizationSourceNequi(nequiInfo) {
    const response = await (0, _nodeFetch.default)(`${paymentGateway}/tokens/nequi`, {
      method: 'POST',
  ...
  async function createPaymentSource(type, token, email, acceptanceToken) {
    const response = await (0, _nodeFetch.default)(`${paymentGateway}/payment_sources`, {
      method: 'POST',
      body: JSON.stringify({
        type,
        token,
      ...
      headers: {
        Authorization: `Bearer ${_config.config.payPrivateKey}`
      }
    });
    const data = await response.json();
    return data;
  ...
  async function insertSourceCard(sourceInfo) {
    ...
    const {
      acceptance_token: acceptanceToken
    } = await generateAcceptanceToken(); // Second tokenizate card info
    ...
    } = await tokenizationSourceCard(cardSourceInfo); // Then create a payment source

    const paymentSource = await createPaymentSource('CARD', token, sourceInfo.email, acceptanceToken); // Finally save the record in the database
  ...
  async function insertSourceNequi(sourceInfo) {
    ...
    } = await tokenizationSourceNequi(nequiSourceInfo); // Then create a payment source

    const paymentSource = await createPaymentSource('NEQUI', token, sourceInfo.email, acceptanceToken); // Finally save the record in the database
  ...
  async function makeTransaction(paymentInfo) {
    ...
    const response = await (0, _nodeFetch.default)(`${paymentGateway}/transactions`, {
      method: 'POST',
