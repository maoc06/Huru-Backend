export default function makeUpdateDefaultPayment({ paymentUserDb }) {
  return async function updateDefaultPayment({ paymentData } = {}) {
    ...
    const oldPayment = { id: oldDefaultId, isDefault: false };
    const newPayment = { id: newDefaultId, isDefault: true };

    await paymentUserDb.updatePayment(oldDefaultId, oldPayment);

    const paymentMethod = await paymentUserDb.updatePayment(
      newDefaultId,
      newPayment
    );
